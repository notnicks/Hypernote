...
function About({ theme, onClose }) {
  ...
  return (
    <div
      className={`flex flex-col h-full overflow-y-auto p-8 max-w-2xl mx-auto ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`}
    >
      <div className="flex items-center gap-4 mb-8">
        <div
          className={`p-4 rounded-2xl ${theme === 'dark' ? 'bg-slate-800' : 'bg-white shadow-sm border border-slate-200'}`}
        >
      ...
      <div className="space-y-6">
        <section>
          <p className="text-lg leading-relaxed">
            Hypernote is a handy little text editor, the one I always wanted but never got round to
            building. It supports markdown and a little mermaid. Oh it has some handy network views,
            wiki linking and tagging too.
          </p>
        </section>

        <div className={`grid grid-cols-1 md:grid-cols-2 gap-4`}>
          <div
            className={`p-4 rounded-xl border ${theme === 'dark' ? 'border-slate-700 bg-slate-800/50' : 'border-slate-200 bg-white'}`}
          >
        ...
        <section
          className={`p-6 rounded-2xl ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-100'}`}
        >
        ...
        <section
          className={`p-6 rounded-2xl ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-100'}`}
        >
          <h2 className="text-xl font-bold mb-4">Open Source Libraries &amp; Plugins</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-4 text-sm">
            <div>
              <span
                className={`block font-semibold ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}
              >
                Excalidraw
              </span>
        ...
        <footer className="pt-8 flex flex-col items-center gap-4">
          <p className="flex items-center gap-2 text-sm italic opacity-70">
            Made with <Heart size={16} className="text-red-500 fill-red-500" /> by the Hypernote
            team
          </p>
          <button
            onClick={onClose}
